...
class InterpolationView(BaseView):
    """
        Interpolate time series data including
        jalali(shamsi) time series and gregorian(miladi) time series
        ...
        Supported time values are:
         'daily', 'monthly', 'hourly', 'minutes',
         '5 minutes' or '5minutes' or 'five minutes'
        Supported interpolation values which is interpolation method are:
         'linear' and 'spline'
    """
...
class JalaliInterpolationView(BaseView):
    """
        ...
        Supported interpolation values which is interpolation method are:
         'linear' and 'spline'
        Supported skip_holiday values which skips thursdays and fridays are:
         true, false (boolian)
    """
...
class OutlierDetectionView(BaseView):
    ...
    def create(self, request, *args, **kwargs):
        ...
        dict_data_outlier_detected = OutlierDetection(
            serializer.data
        ).outlier_detected_data

        return Response(
            dict_data_outlier_detected,
            status=status.HTTP_200_OK,
            headers=headers)
...
class ImbalancedClassificationView(BaseView):
    """
        ...
        Config containing method which can be:
    ...
    def create(self, request, *args, **kwargs):
        ...
        try:
            dict_data_balanced = ImbalancedClassification(
                serializer.data
            ).balanced_data
        except ValueError as e:
            return Response(
